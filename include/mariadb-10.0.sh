...
if [ "$IPADDR_STATE"x == "TW"x ] || [ "$IPADDR_STATE"x == "US"x ];then
    ...
    [ -z "$MARAIDB_TAR_MD5" ] && { DOWN_ADDR_MARIADB=http://sfo1.mirrors.digitalocean.com/mariadb/mariadb-${mariadb_10_0_version}/bintar-${GLIBC_FLAG}-$SYS_BIT_a; MARAIDB_TAR_MD5=`curl -Lk $DOWN_ADDR_MARIADB/md5sums.txt | grep $FILE_NAME | awk '{print $1}'`; }
else
