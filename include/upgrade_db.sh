...
if [ "$DB_tmp" == 'MariaDB' ];then
    [ "$IPADDR_COUNTRY"x == "CN"x ] && DOWN_ADDR=https://mirrors.tuna.tsinghua.edu.cn/mariadb || DOWN_ADDR=http://sfo1.mirrors.digitalocean.com/mariadb
    DB=MariaDB
    ...
    [ "$IPADDR_COUNTRY"x == "CN"x ] && DOWN_ADDR=http://mirrors.sohu.com/mysql || DOWN_ADDR=http://mysql.he.net/Downloads
    DB=MySQL
    OLD_DB_version=$OLD_DB_version_tmp
fi
